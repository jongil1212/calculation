...
st.title("📘 중학교 1학년 정수와 유리수 자동 채점기")
st.write("서논술형 답안을 입력하면 문항별 채점 기준에 따라 자동 채점합니다.")
...
with col1:
    q1_1 = st.text_input("1-(1) 음수", placeholder="-4, -3/5, -6/3")
    q1_2 = st.text_input("1-(2) 정수가 아닌 유리수", placeholder="1/2, -3/5, 3.6")

with col2:
    q1_3 = st.text_input("1-(3) 절댓값이 같은 두 수", placeholder="+2, -6/3")
    q1_4 = st.text_input("1-(4) 작은 수부터 부등호로 나열", placeholder="-4<-6/3<-3/5<0<1/2<2<3.6<10")

st.divider()

# 2번
st.header("2번 계산 순서 오류 수정")

q2_1 = st.text_area(
    "2-(1) 현우의 풀이를 고치고 이유를 쓰시오.",
    placeholder="현우는 나눗셈보다 덧셈을 먼저 해서 틀렸다. 5+(-6)÷2=5+(-3)=2"
)

q2_2 = st.text_area(
    "2-(2) 지수의 풀이를 고치고 이유를 쓰시오.",
    placeholder="지수는 곱셈을 먼저 하지 않아서 틀렸다. 5×(-6)-2×(-3)=-30-(-6)=-24"
)

st.divider()

# 3번
st.header("3번 거듭제곱과 부호")

q3 = st.text_area(
    "3번 답안",
    placeholder="옳은 사람은 건우이다. -2^4=-16이므로 음수이고, 음수 2개를 곱하므로 결과의 부호는 양수이다."
)

st.divider()

# 4번
st.header("4번 사다리 계산 게임")

col3, col4 = st.columns(2)

with col3:
    q4_jinho = st.text_area(
        "진호의 식과 최종 점수",
        placeholder="2×3÷1/2-3/2=21/2"
    )
    q4_haein = st.text_area(
        "해인의 식과 최종 점수",
        placeholder="(-1)÷2×(-1/4)-3/2=-11/8"
    )

with col4:
    q4_seunghye = st.text_area(
        "승혜의 식과 최종 점수",
        placeholder="1÷2×3+7=17/2"
    )
    q4_minseop = st.text_area(
        "민섭의 식과 최종 점수",
        placeholder="(-3)×(-1/4)÷1/2+7=17/2"
    )

q4_final = st.text_input(
    "간식을 사게 될 학생",
    placeholder="해인"
)
...
if st.button("자동 채점하기", type="primary"):
    total_score = 0
    all_scores = {}
    all_feedback = {}
    ...
    q2_total, q2_scores, q2_feedback = grade_q2(q2_1, q2_2)
    q3_total, q3_scores, q3_feedback = grade_q3(q3)
    ...
    st.subheader("문항별 점수")
    st.write(f"1번: {q1_total} / 4점")
    st.write(f"2번: {q2_total} / 4점")
    st.write(f"3번: {q3_total} / 3점")
    st.write(f"4번: {q4_total} / 9점")

    st.subheader("세부 채점 결과")
    ...
    st.subheader("교사용 검토 안내")
    st.info(
        "이 자동 채점기는 정규식과 핵심 의미어 기반으로 작동합니다. "
        "학생 표현이 매우 다양하거나 식을 말로 길게 설명한 경우에는 교사의 최종 검토가 필요합니다."
    )
